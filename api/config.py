...
def _get_provider_base_url(provider_id):
    ...
    For custom providers, also searches ``custom_providers[]`` by slug.
    When multiple custom_providers entries share the same slugified name,
    ...
    # Search custom_providers by slug
    if str(provider_id).startswith("custom:"):
        pid = str(provider_id).lower()
        slug = _custom_provider_slug_from_name(pid)
        matching = [
            entry for entry in _custom_provider_entries()
            if _custom_provider_slug_from_name(str(entry.get("name") or "").strip()) == slug
        ]
        if len(matching) == 1:
            base = (matching[0].get("base_url") or "").strip().rstrip("/")
            if base:
                return base
        elif len(matching) > 1:
            return None  # ambiguous slug — fail closed
...
def resolve_custom_provider_connection(provider_id: str) -> tuple[str | None, str | None]:
    ...
    # Use the shared slugger — handles colons to hyphens consistently
    slug = _custom_provider_slug_from_name(pid)
    if not slug:
        return None, None
    ...
    # Collect all slug-matching entries
    matching = []
    for entry in custom_providers:
        if not isinstance(entry, dict):
            continue
    ...
    # On a slug collision, disambiguate by base_url
    if len(matching) == 1:
        entry = matching[0]
    ...
    # Multiple entries share the same slug — use base_url as second discriminator
    if len(matching) > 1:
        url_entries = [e for e in matching if (e.get("base_url") or "").strip()]
